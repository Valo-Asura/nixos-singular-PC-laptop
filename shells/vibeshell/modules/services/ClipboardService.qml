...
QtObject {
    ...
    function list() {
        ...
        listProcess.command = ["sh", "-c", 
            "sqlite3 '" + dbPath + "' <<'EOSQL'\n.timeout 5000\nDELETE FROM clipboard_items WHERE pinned = 0 AND updated_at < (strftime('%s', 'now', '-2 days') * 1000);\nDELETE FROM clipboard_items WHERE pinned = 0 AND id NOT IN (SELECT id FROM clipboard_items WHERE pinned = 0 ORDER BY display_index ASC, updated_at DESC, id DESC LIMIT 20);\nWITH reindexed AS (SELECT id, ROW_NUMBER() OVER (ORDER BY updated_at DESC, id DESC) - 1 AS new_idx FROM clipboard_items WHERE pinned = 0) UPDATE clipboard_items SET display_index = (SELECT new_idx FROM reindexed WHERE reindexed.id = clipboard_items.id) WHERE pinned = 0;\n.mode json\nSELECT id, mime_type, preview, is_image, binary_path, content_hash, size, created_at, pinned, alias, display_index FROM clipboard_items ORDER BY pinned DESC, display_index ASC, updated_at DESC, id DESC LIMIT 20;\nEOSQL"
        ];
