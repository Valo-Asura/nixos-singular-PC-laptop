# Shared VibeShell source helper: optional upstream NixOS module, not imported by hosts.
{ config, lib, pkgs, ... }:

let
  cfg = config.programs.vibeshell;
in {
  options.programs.vibeshell = {
    enable = lib.mkEnableOption "Vibeshell shell";

    package = lib.mkOption {
      type = lib.types.package;
      description = "The Vibeshell package to use";
    };

    fonts.enable = lib.mkOption {
      type = lib.types.bool;
      default = true;
      description = "Whether to install Vibeshell fonts (including Phosphor Icons)";
    };
  };
